  ...
  getArgument (node, argument = EventArgumentGenerator.next().value) {
    switch (node.type) {
      case Syntax.Identifier:
        if (node.name in this.scope && this.scope[node.name] !== undefined) {
          argument.name = this.scope[node.name].raw || node.name;
          argument.type = this.scope[node.name].type || Type.any;

          if (argument.type in Syntax) {
            argument.type = Type.any;
          }
        } else {
